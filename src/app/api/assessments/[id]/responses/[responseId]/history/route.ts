...
export async function GET(
  ...
  const response = await prisma.assessmentSsdfTaskResult.findUnique({
    where: { id: params.responseId },
    include: { assessment: { select: { id: true, organizationId: true, deletedAt: true } } }
  });

  if (!response || response.assessmentId !== params.id || response.assessment.deletedAt) {
    return NextResponse.json({ error: "Resposta nao encontrada" }, { status: 404 });
  }

  const hasAccess = await ensureOrganizationAccess(session, response.assessment.organizationId);
  if (!hasAccess) {
  ...
  const history = await prisma.assessmentTaskHistory.findMany({
    where: {
      assessmentTaskResultId: response.id,
